# ⚠️ Syntax Error Example 5 – Missing Return Value in a Function  
...
However, if the `return` statement is written **without returning a value**, the compiler should report a **syntax error**.

---

## ❌ Incorrect Code
```java
class SyntaxErrorExample5 {
...
This causes a **compile-time Syntax error**, for example:
```
...
public class SyntaxErrorExample5Solution {
    static int max(int i, int j) {
        if (i > j)
...
- Omitting a return value causes a **Syntax error**, even if your syntax is correct.  
